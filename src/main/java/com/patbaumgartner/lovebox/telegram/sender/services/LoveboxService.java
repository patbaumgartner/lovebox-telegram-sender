...
public class LoveboxService {
	...
	public void simulateDeviceQueries() {
		if (restClientProperties.isEnabled()) {
			...
			GraphqlRequestBody setDeviceGraphqlRequestBody = new GraphqlRequestBody("setDevice", setDeviceVariables,
					setDeviceQuery);
			ResponseEntity<String> setDeviceResponse = restClient.graphql("Bearer " + token,
					setDeviceGraphqlRequestBody);
			...
			ResponseEntity<String> setBoxSignatureResponse = restClient.graphql("Bearer " + token,
					setBoxSignatureGraphqlRequestBody);
	...
	public Tripple<String, LocalDateTime, String> sendImageMessage(String imageAsBase64) {
		if (restClientProperties.isEnabled()) {
			...
			GraphqlRequestBody sendPixNoteGraphqlRequestBody = new GraphqlRequestBody("sendPixNote",
					sendPixNoteVariables, sendPixNoteQuery);
			ResponseEntity<String> sendPixNoteResponse = restClient.graphql("Bearer " + token,
	...
	public String receiveWaterfallOfHearts() {
		if (restClientProperties.isEnabled()) {
			...
			GraphqlRequestBody getHeartsRainGraphqlRequestBody = new GraphqlRequestBody("getHeartsRain",
					getHeartsRainQueryVariables, getHeartsRainQuery);
			ResponseEntity<String> getHeartsRainResponse = restClient.graphql("Bearer " + token,
					getHeartsRainGraphqlRequestBody);
			...
			if (!getHeartsRainString.isJsonNull()) {
				...
				GraphqlRequestBody setHeartsRainGraphqlRequestBody = new GraphqlRequestBody("setHeartsRain",
						setHeartsRainQueryVariables, setHeartsRainQuery);
				ResponseEntity<String> setHeartsRainResponse = restClient.graphql("Bearer " + token,
						setHeartsRainGraphqlRequestBody);
	...
	public List<Pair<String, String>> getMessages() {
		...
		if (restClientProperties.isEnabled()) {
			...
			GraphqlRequestBody getMessagesByBoxGraphqlRequestBody = new GraphqlRequestBody("getMessages",
					getMessagesQueryVariables, getMessagesQuery);
			ResponseEntity<String> getMessagesResponse = restClient.graphql("Bearer " + token,
					getMessagesByBoxGraphqlRequestBody);
			...
			JsonElement getMessagesByBox = jsonRoot.getAsJsonObject().get("data").getAsJsonObject().get("getMessages");

			if (!getMessagesByBox.isJsonNull()) {
				JsonArray messages = getMessagesByBox.getAsJsonArray();
